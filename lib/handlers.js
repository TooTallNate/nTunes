...
module.exports = function(nClass) {
  ...
  nClass["track"].addHandler("raw", function(req, res) {
    ...
    var locationSpecifier = new nSpecifier(this.input.substring(0, this.input.lastIndexOf("/")+1)+"location,kind");
    ...
    applescript.execString('tell application "iTunes"\n'+locationSpecifier.vars+'get '+locationSpecifier.finalVar+'\nend tell\n', function(err, rtn) {
      ...
      helpers.serveStaticFile(req, res, rtn[0]);
  ...
  // "radio-redirecter.js"). Once the redirected URL is known, "radio-playlist-parser.js"
  // retrieves the remote file and parses it, ultimately returning an Array
  // of absolute URLs to infinite audio streams of the 'URL track'.
