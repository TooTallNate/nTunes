...
function nSpecifier(specifier) {
  ...
  while (i < this.chunks.length) {
    ...
    if (this._props.length==1) {
      ...
      // property request.
      var parentType = currentObj.getProperty(prevChunk).type;
      if (parentType == 'specifier') {
        isList = true;
        parentType = 'track';
      }
      var parentClass = nClass.getClass(parentType);
      if (parentClass) {
    ...
    if (currentObj.hasElement(next)) {
      ...
      var handler = currentObj.getHandler(next);
      if (handler) {
...
//   '/source/-40/playlist/-260/file track/-263'
