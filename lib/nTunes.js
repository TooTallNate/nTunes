...
require.paths.unshift(require("path").join(__dirname, "../vendor/node-applescript/lib"));
var fs = require("fs");
var sys = require("sys");
var Url = require("url");
...
nTunes.prototype.handleNTunesRequest = function(req, res, next) {
  ...
  if (req.method == "POST" && req.parsedUrl.request.length == 1 && nCommand.REGEXP.test(req.parsedUrl.request[0])) {
    ...
    try {
      resolvedSpecifier = new nSpecifier(req.parsedUrl.decodedPath);
    } catch (e) {
    ...
    if (req.method == "POST") {
      ...
      as += 'get ' + resolvedSpecifier.properties +
        (resolvedSpecifier.properties &&  resolvedSpecifier.finalVar? ' of ' : '') +
        (resolvedSpecifier.finalVar ? resolvedSpecifier.finalVar : '');
      self.doAppleScriptThenResponse(as, res);
    }    
  }
