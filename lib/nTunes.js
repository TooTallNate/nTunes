...
nTunes.prototype.doNTunesCommand = function(command, params, res, url) {
  ...
  asStr = 'tell application "iTunes" to\n'+asStr+'\nend tell';
  console.log(asStr);
...
nTunes.prototype.handleNTunesRequest = function(req, res, next) {
  ...
  if (req.method == "POST" && url.request.length == 1 && nCommand.REGEXP.test(url.request[0])) {
  ...
  try {
    var resolvedSpecifier = new nSpecifier(url.decodedPath);
    isValidApiRequest = true;
    console.log(resolvedSpecifier.toAppleScript());
